...
// Handle form submission for creating or updating banner slot
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] == UPLOAD_ERR_OK) {
        ...
        // Create a directory for this specific banner using the banner's title or ID
        ...
        $bannerSlot->image = isset($bannerSlot->image) ? $bannerSlot->image : null;
    }

    // Create or update the banner slot
    if ($bannerSlot->id) {
        ...
        if ($bannerSlot->create()) {
            header("Location: banner_slots.php");
        } else {
            echo "Unable to create banner slot.";
        }
    }
}
...
<h2><?php echo isset($bannerSlot->id) ? 'Edit Banner Slot' : 'Create Banner Slot'; ?></h2>
<form action="create_banner.php<?php echo isset($bannerSlot->id) ? '?id=' . $bannerSlot->id : ''; ?>" method="post" enctype="multipart/form-data">
    ...
    <button type="submit"><?php echo isset($bannerSlot->id) ? 'Update Banner Slot' : 'Create Banner Slot'; ?></button>
</form>
